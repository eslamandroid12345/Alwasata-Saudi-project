...
@section('title')
{{ MyHelpers::admin_trans(auth()->user()->id,'Need To be Turned Requests') }}
@endsection
...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        <h3>{{ MyHelpers::admin_trans(auth()->user()->id,'Need To be Turned Requests') }}:</h3>
    </div>
</div>
<br>
...
<script>
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ url('qualityManager/needToBeTurnedReq-datatable') }}",
                'data': function(data) {
            ...
            columns: [
                ...
                {
                    data: 'status',
                    name: 'quality_request_need_turneds.status'
                },
                {
                    data: 'quacomment',
            ...
            createdRow: function(row, data, index) {
                ...
                $('td', row).eq(4).attr('title', data.quacomment); // to show other text of comment

                $('td', row).eq(5).addClass('commentStyle'); // 6 is index of column
                $('td', row).eq(5).attr('title', data.reject_reason); // to show other text of comment

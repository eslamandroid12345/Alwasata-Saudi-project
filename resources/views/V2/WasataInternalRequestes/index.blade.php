    ...
    <div class="dashTable">
        <table class="table table-bordred table-striped data-table">
            ...
            <tr>
                ...
                <th style="text-align:center">{{ MyHelpers::admin_trans(auth()->user()->id,'req num') }}</th>

                <th>{{ MyHelpers::admin_trans(auth()->user()->id,'Funding Manager') }}</th>
                <th>{{ MyHelpers::admin_trans(auth()->user()->id,'Customer') }}</th>
                <th>{{ MyHelpers::admin_trans(auth()->user()->id,'mobile') }}</th>
                <th style="text-align:center">{{ MyHelpers::admin_trans(auth()->user()->id,'req type') }} {{-- <br /> {{ MyHelpers::admin_trans(auth()->user()->id,'in Bank User') }} --}}</th>
                {{-- <th style="text-align:center">{{ MyHelpers::admin_trans(auth()->user()->id,'req status') }} <br /> {{ MyHelpers::admin_trans(auth()->user()->id,'in Funding Manager') }}</th> --}}
                {{-- <th>{{ MyHelpers::admin_trans(auth()->user()->id,'agent comment') }}</th> --}}
                {{-- <th>{{ MyHelpers::admin_trans(auth()->user()->id,'req classification') }}</th> --}}
                <th>{{ MyHelpers::admin_trans(auth()->user()->id,'Bank User Comment') }}</th>
 ...
 $('.tokenizeable').tokenize2();
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ route('V2.ExternalCustomer.requestes-of-wasata-internal-indexDatatable') }}",
                'data': function(data) {
            ...
            columns: [
                ...
                {
                    "targets": 0,
                    "name": "wasata_requestes.created_at",
                    "data": "created_at",
                    "render": function(data, type, row, meta) {
                        return data.split(" ").join("<br/>");
                    }
                ...
                {
                    data: 'type',
                    name: 'wasata_requestes.req_type'
                },
                // {
                //     data: 'statusReq',
    ...
    function savecomm(reqID) {
        ...
        $.ajax({
            url: "{{ route('V2.ExternalCustomer.updatecomm') }}",
            type: "POST",
            data: {
                "_token": "{{csrf_token()}}",
                id: id,

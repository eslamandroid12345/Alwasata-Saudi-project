...
@section('customer')
    ...
    <form action="{{ route('V2.BankDelegate.request.update')}}" method="post" novalidate="novalidate" id="frm-update">
    ...
    @csrf
                        ...
                        <div class="col-lg-10">
                            <div class="tableUserOption   flex-wrap justify-content-md-end  justify-content-center downOrder">
                                <div class="addBtn  mt-lg-0 mt-3 orderBtns">
                                    ...
                                    @if ($w_req = \App\Models\WasataRequestes::where('user_id',auth()->id())->where('req_id', $purchaseCustomer->id)->first() )
                                        <button class="mr-3 DarkRed  item">
                                            @if ($w_req->req_status == '1')
                                                    <a href="{{route('V2.BankDelegate.request.internal-archive',$purchaseCustomer->id)}}" class="text-white" style="text-decoration: none" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Archive') }}">
                                                        <i class="fas fa-stop"></i>
                                                        {{ MyHelpers::admin_trans(auth()->user()->id,'Archive') }}
                                                    </a>

                                                    @else
                                                    <a href="{{route('V2.BankDelegate.request.internal-not-archive',$purchaseCustomer->id)}}" class="text-white" style="text-decoration: none" title=" ارجاع من الارشفة">
                                                        <i class="fas fa-stop"></i>
                                                        ارجاع من الارشفة
                                                    </a>
        ...
        <section class="new-content pt-0">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                            <ul class="list-unstyled d-flex flex-wrab align-items-center justify-content-center topRow">
                                <li id="content3" class="tab @if(session()->has('message5')) active-on @endif ">
                            ...
                            <div class="tab-body">
                                <div class="row hdie-show @if(session()->has('message5')) display-flex @endif " id="content4-cont">
                                    <div class="col-lg-12   mb-md-0 ">
                                        @include('V2.ExternalCustomer.Request.single.fundingCustomer')
                                    </div>
                                </div>
                                ...
                                <div class="row hdie-show   display-none " id="content1-cont">
                                    <div class="col-lg-12 mb-5 mb-md-0">
                                        @include('V2.ExternalCustomer.Request.single.document')
                                    </div>
                                </div>
                            </div>
                            @include('V2.ExternalCustomer.Request.single.fundingReqInfo')
                        </div>
                    </div>
                </div>
            </div>
...
@endsection
@section('updateModel')
    @include('V2.ExternalCustomer.Request.single.req_records')
    @include('V2.ExternalCustomer.Request.single.documentModel')
    @include('Helpers.addPhone')
@endsection

@section('confirmMSG')
    @include('V2.ExternalCustomer.Request.single.confirmationMsg')
    @include('V2.ExternalCustomer.Request.single.confirmSendingMsg')
    @include('V2.ExternalCustomer.Request.single.confirmSendingMsgPay')
@endsection

@section('scripts')
    @include('FundingCalculater.calculaterJS')
  {{--  @include('V2.ExternalCustomer.FundingCalculater.calculaterJS')--}}
    <script>
        //---------------------to show wraning msg---------------
        ...
        $('#file-form').submit(function (event) {
            ...
            $.ajax({
                ...
                url: "{{ route('V2.BankDelegate.request.uploadFile')}}",
                data: formData,
                type: 'post',
                async: false,
                processData: false,
        ...
        $(document).on('click', '#delete', function (e) {
            ...
            modalConfirm(function (confirm) {
                if (confirm) {
                    $.post("{{ route('V2.BankDelegate.delete.file') }}", {
                        id: id

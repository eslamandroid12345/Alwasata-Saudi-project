<div class="userFormsInfo  ">
    ...
    <div class="userFormsContainer mb-3">
        ...
        @include('MortgageManager.fundingReq.fundingJoint')
    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        var statusMor = document.getElementById("statusMorPur").value;
        ...
        if ((status != 6 && status != 13) || statusPay == 0 || statusPay == null || statusPay == 2 || statusPay == 8 || statusPay == 9) {
            if (status == 1) { //in sales agent
                document.getElementById('sendingWarning1').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button> {{ MyHelpers::admin_trans(auth()->user()->id,'The request with Sales Agent, you cannot edit anything') }}";
                document.getElementById('sendingWarning1').style.display = "block";
            }
            ...
            if (status == 11) { //archived in mortgage manager
                document.getElementById('archiveWarning1').innerHTML = "<button type='button' class='close' data-dismiss='alert'>&times;</button> {{ MyHelpers::admin_trans(auth()->user()->id,'The request is archived by Mortgage Manager, you cannot edit anything') }}";
                document.getElementById('archiveWarning1').style.display = "block";
            }
    ...
    $('#file-form').submit(function(event) {
        ...
        $.ajax({
            ...
            url: "{{ route('mortgage.manager.uploadFile')}}",
            data: formData,
            type: 'post',
            async: false,
            processData: false,
            ...
            success: function(response) {
                ...
                $.each(response, function(i, value) { //for loop , I put data as value
                    ...
                    var url = '{{ route("mortgage.manager.openFile", ":docID") }}';
                    url = url.replace(':docID', docID);
                    var url2 = '{{ route("mortgage.manager.downFile", ":docID") }}';
    ...
    $(document).on('click', '#delete', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('mortgage.manager.deleFile') }}", {
                    id: id
    ...
    $(document).on('click', '#send', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('mortgage.manager.sendFunding')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    //  console.log(data);
                    var url = '{{ route("mortgage.manager.fundingRequest", ":reqID") }}';
    ...
    $(document).on('click', '#reject', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                var comment = document.getElementById("comment2").value;
                $.post("{{ route('mortgage.manager.rejectRequest')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    var url = '{{ route("mortgage.manager.fundingRequest", ":reqID") }}';
    ...
    $(document).on('click', '#approveTsaheel', function(e) {
        ...
        $.get("{{ route('mortgage.manager.aprroveTsaheel') }}", {
            id: id
        }, function(data) {
    ...
    $(document).on('click', '#undoTsaheelApprove', function(e) {
        ...
        $.get("{{ route('mortgage.manager.undoaprroveTsaheel') }}", {
            id: id
        }, function(data) {
        ...
        if (statusReq == 6 || statusReq == 13) {
            ...
            if ((statusReq == 6 || statusReq == 13))
                if (status == 8) { //canceled payment
                    document.getElementById('archiveWarning').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button> {{ MyHelpers::admin_trans(auth()->user()->id,'The prepayment canceled') }}   , <a href='{{ route('mortgage.manager.restPrepayment',$id)}}'> {{ MyHelpers::admin_trans(auth()->user()->id,'Recancele') }}؟ </a>";
                    document.getElementById('archiveWarning').style.display = "block";
                }
    ...
    $('#frm-update').on('click', '#updatePay', function(e) {
        ...
        $.post("{{ route('mortgage.manager.updatePrepayment')}}", {
            reqID: reqID,
            real: real,
            incr: incr,
            preval: preval,
    ...
    $(document).on('click', '#appPay', function(e) {
        ...
        var modalConfirm = function(callback) {
            ...
            $("#modal-btn-si8").on("click", function() {
                callback(true);
                $("#mi-modal8").modal('hide');
            });

            $("#modal-btn-no8").on("click", function() {
                callback(false);
                $("#mi-modal8").modal('hide');
            });
        };
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.get("{{ route('mortgage.manager.appPrepayment')}}", {
                    id: id
                }, function(data) {
                    var url = '{{ route("mortgage.manager.fundingRequest", ":reqID") }}';
                    url = url.replace(':reqID', data.id);
    ...
    $(document).on('click', '#getPay', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.get("{{ route('mortgage.manager.getPrepayment')}}", {
                    id: id
                }, function(data) {
                    var url = '{{ route("mortgage.manager.fundingRequest", ":reqID") }}';
                    url = url.replace(':reqID', data.id);
    ...
    $(document).on('click', '#rejectPay', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                if (comment != '') {
                    $("#mi-modal3").modal('hide');
                    $.get("{{ route('mortgage.manager.rejectPrepayment')}}", {
                        id: id,
                        comment: comment
                    }, function(data) {
                        console.log(data);

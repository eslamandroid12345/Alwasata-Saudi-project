<div class="userFormsInfo">
    ...
    <div class="userFormsContainer mb-3">
        <div class="userFormsDetails topRow">
            @if ($reqStatus != 23 && $reqStatus != 25 && $reqStatus != 26 && $reqStatus != 27)
        ...
        @include('MortgageManager.morPurReq.fundingJoint')
    </div>
</div>
...
*/
        ...
        if (status == 24) { //canceled
            document.getElementById('archiveWarning').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button>  {{ MyHelpers::admin_trans(auth()->user()->id,'The request has canceled') }}.. <a href='{{ route('mortgage.manager.restMorPur',$id)}}'> {{ MyHelpers::admin_trans(auth()->user()->id,'Recancele') }}?</a>";
            document.getElementById('archiveWarning').style.display = "block";
        }
    ...
    $(document).on('click', '#send', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('mortgage.manager.sendMorPur')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    //  console.log(data);
                    var url = '{{ route("mortgage.manager.morPurRequest", ":reqID") }}';
    ...
    $(document).on('click', '#complete', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                document.querySelector('#complete').innerHTML = "<i class='fa fa-spinner fa-spin'></i>{{ MyHelpers::admin_trans(auth()->user()->id,'Loading') }}";

                $.get("{{ route('mortgage.manager.completeMorPur')}}", {
                    id: id
                }, function(data) {
                    ...
                    if (data.status == 1) {
                        window.location.href = url; //using a named route
                        document.querySelector('#complete').innerHTML = "<i class='fa fa-check'></i>{{ MyHelpers::admin_trans(auth()->user()->id,'Completed') }}";
    ...
    $(document).on('click', '#undocomplete', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $('#undocomplete').attr("disabled", true);
                document.querySelector('#undocomplete').innerHTML = "<i class='fa fa-spinner fa-spin'></i>{{ MyHelpers::admin_trans(auth()->user()->id,'Loading') }}";

                $.get("{{ route('mortgage.manager.undocompleteMorPur')}}", {
                    id: id
                }, function(data) {
                    ...
                    if (data.status == 1) {
                        window.location.href = url; //using a named route
                        document.querySelector('#undocomplete').innerHTML = "<i class='fa fa-check'></i>{{ MyHelpers::admin_trans(auth()->user()->id,'Undo complete') }}";
                        $('#undocomplete').attr("disabled", false);

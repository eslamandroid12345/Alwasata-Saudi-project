...
@include('GeneralManager.Request.confirmArchMsg')
@endsection
    ...
    function archiveAllReqs(array) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('general.manager.archReqArray')}}", {
                    array: array,
                    _token: "{{csrf_token()}}",
                }, function(data) {

                    var url = '{{ route("general.manager.recivedRequests") }}';
...
<script>
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ url('generalmanager/recivedreqs-datatable')  }}",
                'method': 'Get',
                'data': function(data) {
            ...
            createdRow: function(row, data, index) {
                ...
                $('td', row).eq(8).attr('title', data.gm_comment); // to show other text of comment

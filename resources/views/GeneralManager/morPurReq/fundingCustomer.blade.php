<div class="userFormsInfo">
    ...
    <div class="userFormsContainer mb-3">
        ...
        @include('GeneralManager.morPurReq.fundingJoint')
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        if (status == 27) { //canceled
            document.getElementById('archiveWarning').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button>   {{ MyHelpers::admin_trans(auth()->user()->id,'The request has canceled') }} .. <a href='{{ route('general.manager.restMorPur',$id)}}'> {{ MyHelpers::admin_trans(auth()->user()->id,'Recancel?') }}</a>";
            document.getElementById('archiveWarning').style.display = "block";
        }
    ...
    $(document).on('click', '#approve', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('general.manager.appMorPur')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    //  console.log(data);
                    var url = '{{ route("general.manager.morPurRequest", ":reqID") }}';
    ...
    $(document).on('click', '#reject', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('general.manager.rejectMorPur')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    var url = '{{ route("general.manager.morPurRequest", ":reqID") }}';

...
@section('title')
{{ MyHelpers::admin_trans(auth()->user()->id,'Accounting Report') }} {{ MyHelpers::admin_trans(auth()->user()->id,'to wsata') }} - تحت المعالجة
@endsection
...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        <h3>{{ MyHelpers::admin_trans(auth()->user()->id,'Accounting Report') }} {{ MyHelpers::admin_trans(auth()->user()->id,'to wsata') }}  - تحت المعالجة :</h3>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ url('report/wsataAccountingUnderReport_datatable') }}",
                'data': function(data) {

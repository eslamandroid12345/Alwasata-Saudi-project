...
@section('title')
{{ MyHelpers::admin_trans(auth()->user()->id,'Accounting Report') }} {{ MyHelpers::admin_trans(auth()->user()->id,'to tsaheel') }}  - تحت المعالجة
@endsection
...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        <h3>{{ MyHelpers::admin_trans(auth()->user()->id,'Accounting Report') }} {{ MyHelpers::admin_trans(auth()->user()->id,'to tsaheel') }} - تحت المعالجة:</h3>
    </div>
</div>
...
$.ajax({
    ...
    function saveTsaheelMortgage(reqID) {
...
<script>
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ url('report/tsaheelAccountingUnderReport-datatable') }}",
                'data': function(data) {

...
@section('title')
    {{ MyHelpers::admin_trans(auth()->user()->id,'first_batch_calculator_setting') }}
@endsection
@section('css_style')
...
@section('customer')
    ...
    <div class="addUser my-4">
        <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
            <h3> {{ MyHelpers::admin_trans(auth()->user()->id,'first_batch_calculator_setting') }}</h3>
        </div>
    </div>
    <br>
    @if ($firstBatch > 0)
                    ...
                    <div class="col-lg-9">
                        <div class="tableUserOption  flex-wrap ">
                            ...
                            <div class="addBtn col-md-5 mt-lg-0 mt-3">
                                <a href="{{route('admin.addNewFirstBatchItem')}}">
            ...
            <div class="dashTable">
                <table id="pendingReqs-table" class="table table-bordred table-striped data-table">
                    ...
                    <tr>
                        ...
                        <th>المسكن </th>
                        <th>النسبة </th>
                        <th>قيمة العقار - من </th>
                        <th>قيمة العقار - إلي </th>
                        <th>مضمون </th>
    ...
    <script>
        $(document).ready(function() {
            var dt = $('.data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ url('admin/first-batch-datatable') }}",
                columns: [
                    {
                        data: 'id',
                        name: 'id'
                    ...
                    {
                        data: 'residence_type_to_string',
                        name: 'residence_type_to_string'
                    },
                    {
                        data: 'percent_to_string',
                        name: 'percent_to_string'
                    },
                    {
                        data: 'from_property_amount_to_string',
                        name: 'from_property_amount_to_string'
                    },
                    {
                        data: 'to_property_amount_to_string',
                        name: 'to_property_amount_to_string'
                    },
                    {
                        data: 'secured_to_string',
                        name: 'secured_to_string'
                    },
                    {
                        data: 'action',
                        name: 'action'
        ...
        $(document).on('click','#remove', function (){
            ...
            $('#ok_button').click(function (){
                $.ajax({
                    url: "{{ url('/admin/first-batch-remove/') }}",
                    type: "POST",

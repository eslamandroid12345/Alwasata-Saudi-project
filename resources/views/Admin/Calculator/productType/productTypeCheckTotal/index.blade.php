...
@section('title')
    {{ MyHelpers::admin_trans(auth()->user()->id,'product_type_check_total') }}
@endsection
@section('css_style')
...
@section('customer')
    ...
    <div class="addUser my-4">
        <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
            <h3> {{ MyHelpers::admin_trans(auth()->user()->id,'product_type_check_total') }}</h3>
        </div>
    </div>
    <br>
    @if ($productTypeCheckTotal > 0)
        <div class="tableBar">
            <div class="topRow">
                    ...
                    <div class="col-lg-9">
                        <div class="tableUserOption  flex-wrap ">
                            ...
                            <div class="addBtn col-md-5 mt-lg-0 mt-3">
                                <a href="{{route('admin.addNewProductTypeCheckTotalItem')}}">
    ...
    <script>
        $(document).ready(function() {
            var dt = $('.data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ url('admin/product-type-check-total-datatable') }}",
                columns: [
                    {
                        data: 'id',
                        name: 'id'
                    ...
                    {
                        data: 'product_type_id_to_string',
                        name: 'product_type_id_to_string'
                    },
                    // {
                    //     data: 'first_batch_percentage_to_string',
                    //     name: 'first_batch_percentage_to_string'
                    // },
                    {
                        data: 'percentage_to_string',
                        name: 'percentage_to_string'
                    },
                    {
        ...
        $(document).on('click','#remove', function (){
            var product_type_id = $(this).attr('data-id');
            $('#confirmDeleteModal').modal('show');
            $('#ok_button').click(function (){
                $.ajax({
                    url: "{{ url('/admin/product-type-check-total-remove/') }}",
                    type: "POST",
                    data: {  _token: '{{csrf_token()}}', id: product_type_id },

...
@section('customer')
    ...
    <div class="addUser my-4">
        ...
        <h3> مقارنة مقترحات :
            ...
            <br>
                <p style="font-weight: bold;">" {{$name['data']['text']}} "</p>
        </div>
    </div>
    <br>
    ...
    <script>
        $(document).ready(function() {
            var dt = $('.data-table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.compare.percentages.datatable',$apiId) }}",
                columns: [
                    {
                        data: 'id',
                        name: 'id'
        ...
        function ArchiveForm(id) {
            ...
            swal({
                title: 'هل انت متأكد',
                text: "هل انت موافق على رفض المقترح؟",
                type: 'warning',
                showCancelButton: true,
                cancelButtonColor: '#d33',
                confirmButtonColor: '#d33',
                buttons: ["إلغاء","نعم , رفض المقترح !"],
            }).then(function(inputValue) {
                if (inputValue != null) {
                    $.ajax({
                        url: "{{ route('admin.suggestions.archive') }}",
                        type: "POST",
                        data: {
                            '_method': 'POST',
                            '_token': csrf_token,

...
<script>
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{ route('requests.rejections') }}",
            columns: [
    ...
    function addForm() {
        $('#add-form #title-error').html("");
        save_method = "add";
        $('#add-form input[name=_method]').val('POST');
        $('#add-form').modal('show');
        $('#add-form form')[0].reset();
        $('#add-form .modal-title').text('إضافــــــــة سبب الرفض');
    ...
    function editForm(id) {
        ...
        $('#add-form form')[0].reset();
        $('#add-form .modal-title').text('تعديل السبب الرفض');
        $.ajax({
            url: "{{ url('admin/rejections') }}" + '/' + id + "/edit",
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                $('#add-form').modal('show');
                $('#add-form #id').val(data.id);
                $('#add-form #title').val(data.title);
            },
            error: function() {
                alert("Nothing Data");
            }
    ...
    $(function() {
        $('#form-contact').on('submit', function(e) {
            if (!e.isDefaultPrevented()) {
                var id = $('#id').val();
                if (save_method == 'add') url = "{{ url('admin/rejections') }}";
                else url = "{{ url('admin/rejections') . '/' }}" + id;

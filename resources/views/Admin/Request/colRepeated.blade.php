...
@section('customer')
    ...
    <div class="tableBar">
        ...
        <div class="row">
            <div class="col-lg-11">
                <select id="collaborators" class="form-control" style="width: 100%" multiple>
            ...
            <div class="col-lg-1">
                <button type="submit" class="btn btn-primary" id="filter-search-reqs">{{ MyHelpers::admin_trans(auth()->user()->id,'Search') }}</button>
            </div>
        </div>
        <div class="dashTable">
    ...
    <script>
        ...
        $(document).ready(function () {
            datatable = $('.data-table').DataTable({
                ...
                serverSide: true,
                ajax: ({
                    'url': "{{ route('admin.colReqs_datatable',[$id,"repeated"]) }}",
                    'method': 'Get',
                    'data': function (data) {
                ...
                initComplete: function () {
                    ...
                    $("#filter-search-reqs").on('click', function (e) {
                        $('#frm-update')[0].reset();
                        e.preventDefault();
                        api.draw();
                    });
...
<script>
    $("#collaborators").select2();
        function sendRequestToCol(id) {
            var csrf_token = $('meta[name="csrf-token"]').attr('content');
            swal({
                title: 'هل انت متأكد من انزال الطلب',
                text: "هل انت متأكد من انزال الطلب للمتعاون , سوف يظهر الطلب للمتعاون ",
                type: 'warning',
                showCancelButton: true,
                cancelButtonColor: '#d33',
                confirmButtonColor: '#3085d6',
                buttons: ["إلغاء","نعم , انزل الطلب !"],
            }).then(function(inputValue) {
                if (inputValue != null) {
                    $.ajax({
                        url: "{{ url('admin/down-request/') }}" + '/' + id,
                        type: "GET",
                        success: function(data) {
                            datatable.draw();
                            swal({
        ...
        function reverseRequestToCol(id) {
            var csrf_token = $('meta[name="csrf-token"]').attr('content');
            swal({
                title: 'هل انت متأكد من سحب الطلب',
                text: "سوف يتم سحب لاطلب بالكامل من عند المتعاون ولن يمكنه متابعه الطلب",
                type: 'warning',
                showCancelButton: true,
                cancelButtonColor: '#d33',
                confirmButtonColor: '#3085d6',
                buttons: ["إلغاء","نعم , اسحب الطلب !"],
            }).then(function(inputValue) {
                if (inputValue != null) {
                    $.ajax({
                        url: "{{ url('admin/reverse-request/') }}" + '/' + id,
                        type: "GET",
                        success: function(data) {
                            datatable.draw();
                            swal({

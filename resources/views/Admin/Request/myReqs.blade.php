    ...
    <script>
        ...
        $(document).ready(function () {
            datatable = $('.data-table').DataTable({
                ...
                buttons: [
...

                    {
                        ...
                        action: function (e, dt, node, config) {
                            $('#myModal').modal('show');
                            $('#session_remove_button').removeClass('d-none');
                        }
                    }
                ],
                processing: true,
                serverSide: true,
                ajax: ({
                    'url': "{{ url('admin/myreqs-datatable') }}",
                    'method': 'Get',
                    'data': function (data) {
        ...
        $(document).on('click','#session_remove_button',function(){
           // remove sesstion
           $.ajax({
                type:'GET',
                url:"{{ route('admin.remove_session_filter')}}",
                data:{
                    _token: '{{csrf_token()}}'
                },
                success:function(response){
                    $('#agents_ids').tokenize2().trigger('tokenize:clear');
                    $('#classifcation_sa').tokenize2().trigger('tokenize:clear');
                    $('#classifcation_sm').tokenize2().trigger('tokenize:clear');
                    $('#classifcation_fm').tokenize2().trigger('tokenize:clear');
                    $('#classifcation_mm').tokenize2().trigger('tokenize:clear');
                    $('#classifcation_gm').tokenize2().trigger('tokenize:clear');
                    $('#request_status').tokenize2().trigger('tokenize:clear');
                    $('#source').tokenize2().trigger('tokenize:clear');
                    $('#collaborator').tokenize2().trigger('tokenize:clear');
                    $('#notes_status').tokenize2().trigger('tokenize:clear');
                    $('#app_downloaded').tokenize2().trigger('tokenize:clear');
                    $('#region_ip').tokenize2().trigger('tokenize:clear');
                    $('#request_type').tokenize2().trigger('tokenize:clear');
                    $('#pay_status').tokenize2().trigger('tokenize:clear');
                    $('#work_source').tokenize2().trigger('tokenize:clear');
                    $('#salary_source').tokenize2().trigger('tokenize:clear');
                    $('#founding_sources').tokenize2().trigger('tokenize:clear');
                    $('#quality-users').tokenize2().trigger('tokenize:clear');
                    $('#quality_recived').tokenize2().trigger('tokenize:clear');
                    $('#classifcation_qu').tokenize2().trigger('tokenize:clear');


                    $('#customer-salary').val('');
                    $('#customer-salary-to').val('');
                    $('#customer-phone').val('');
                    $('#customer-birth').val('');
                    $('#request-date-from').val('');
                    $('#request-date-to').val('');
                    $('#complete-date-from').val('');
                    $('#complete-date-to').val('');
                    $('#updated_at_from').val('');
                    $('#updated_at_to').val('');

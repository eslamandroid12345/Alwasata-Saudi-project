                ...
                <div class="col-lg-7 offset-md-3">
                    <div class="card">
                        ...
                        <div class="card-body card-block">
                            ...
                            <form action="{{route('nationality.update',$data->id)}}" method="post" id="form-data">
                                @csrf
                                @method('PATCH')
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="status" required>
                                         <option value="1" <?php if($data->status == '1'){echo'selected';}?>>ظاهر فى فورم التوظيف</option>
                                         <option value="0" <?php if($data->status == '0'){echo'selected';}?>>مخفى فى فورم التوظيف</option>
                                    </select>
                                    @error('status')
                                        <small class="form-text text-danger">{{$message}}</small>
                                    @enderror

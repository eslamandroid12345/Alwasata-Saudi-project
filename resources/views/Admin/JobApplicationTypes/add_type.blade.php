            ...
            <div class="row">
               <div class="addBtn col-md-5 mt-lg-0 mt-3">
                ...
                <div class="col-lg-7 offset-md-3">
                    <div class="card">
                        <div class="card-header"> اضافه تصنيف</div>
                        <div class="card-body card-block">

                            <form action="{{route('job_applications_types.store')}}" method="post" id="form-data"  enctype="multipart/form-data" class="">
                                @csrf
                                <div class="form-group">
                                    <label for="managers" class="control-label mb-1">العنوان</label>
                                    <input type="text" class="form-control" name="title"  placeholder="ادخل اسم التصنيف " value="{{old('title')}}" required>
                                    @error('title')
                                    ...
                                    <select class="form-control" name="status" required>
                                         <option value="1" >ظاهر لدى الموارد البشريه</option>
                                         <option value="0" >مخفى لدى الموارد البشريه</option>
                                    </select>
                                    @error('status')
                                        <small class="form-text text-danger">{{$message}}</small>
                                    @enderror

...
<form action="{{ route('sales.manager.updateFunding')}}" method="post" novalidate="novalidate">
        @csrf
            ...
            <div class="topRow no-radius">
                <div class="row align-items-center text-center text-md-left">
                    ...
                    <div class="col-lg-7">
                        <div class="tableUserOption   flex-wrap justify-content-md-end  justify-content-center downOrder">
                            <div class="addBtn  mt-lg-0 mt-3 orderBtns">
                                @if ($purchaseCustomer->isSentSalesAgent ==1 )
                                    <button class="mr-3 warning item" role="button" type="button" id="reject" @if ($reqStatus != 18 && $reqStatus != 22  ) disabled style="cursor: not-allowed" @endif data-id="{{$id}}">
                                        <i class="fas fa-trash mr-2"></i>
                                        {{ MyHelpers::admin_trans(auth()->user()->id,'Reject The Request') }}
                                    </button>
                                @endif

                                <button class="green item" type="submit" id="update" @if ($reqStatus != 18 && $reqStatus != 22  ) disabled style="cursor: not-allowed" @endif    title="{{ MyHelpers::admin_trans(auth()->user()->id,'Update') }}">
                                    <i class="fas fa-edit"></i>
                                    {{ MyHelpers::admin_trans(auth()->user()->id,'Update') }}
                                </button>

                                <button class="mov item" type="button" id="send"  data-id="{{$id}}" @if ($reqStatus != 18 && $reqStatus != 22   ) disabled style="cursor: not-allowed" @endif   title="{{ MyHelpers::admin_trans(auth()->user()->id,'Send The Request') }}">
            ...
            <div class="container-fluid">
                ...
                <div>
                    <label style="width: 100%; display: block;" for="tab1">
                        <span>
                             @include('SalesManager.morPurReq.fundingReqInfo')
                        </span>
                    </label>
...
@section('updateModel')
    @include('SalesManager.morPurReq.req_records')
    @include('SalesManager.morPurReq.documentModel')
@endsection

@section('confirmMSG')
    @include('SalesManager.morPurReq.confirmationMsg')
    @include('SalesManager.morPurReq.confirmSendingMsg')
    @include('SalesManager.morPurReq.confirmRejectMsg')
    @include('SalesManager.morPurReq.SendingModel')
@endsection

...
@section('title')
{{ MyHelpers::admin_trans(auth()->user()->id,'pur-prep') }}
@endsection
...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        <h3>{{ MyHelpers::admin_trans(auth()->user()->id,'pur-prep') }}:</h3>
    </div>
</div>
<br>
...
@else
    <div class="middle-screen">
        <h2 style=" text-align: center;font-size: 20pt;">{{ MyHelpers::admin_trans(auth()->user()->id,'No Requests') }} {{ MyHelpers::admin_trans(auth()->user()->id,'pur-pre') }}</h2>
    </div>
...
<script>
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{ url('salesManager/prepaymentreqs-datatable') }}",
            columns: [
            ...
            createdRow: function(row, data, index) {
                    ...
                    $('td', row).eq(6).attr('title', data.sm_comment); // to show other text of comment

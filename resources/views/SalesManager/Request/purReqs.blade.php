...
@section('title')
{{ MyHelpers::admin_trans(auth()->user()->id,'requests') }}  {{ MyHelpers::admin_trans(auth()->user()->id,'Purchase') }}
@endsection
...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        <h3>{{ MyHelpers::admin_trans(auth()->user()->id,'requests') }}  {{ MyHelpers::admin_trans(auth()->user()->id,'Purchase') }}:</h3>
    </div>
</div>
<br>
...
@section('updateModel')
@include('SalesManager.Request.filterReqs')
@include('SalesManager.Request.confirmArchMsg')
@endsection
    ...
    */
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ url('salesManager/purReqs-datatable') }}",
                'data': function(data) {
            ...
            createdRow: function(row, data, index) {
                ...
                $('td', row).eq(7).attr('title', data.sm_comment); // to show other text of comment

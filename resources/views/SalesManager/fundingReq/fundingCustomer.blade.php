...
<div class="userFormsInfo  ">
    <div class="headER topRow text-center">
        ...
        <div class="userFormsContainer mb-3">
                    ...
                    <div  id="tableAdminOption" class="row">
                        ...
                        <div class="col-12">
                            ...
                            <span id="record" role="button" type="button" class="item span-20"  data-id="mobile" data-toggle="tooltip" data-placement="top" title="{{ MyHelpers::admin_trans(auth()->user()->id,'records')}}">
                                ...
                                <input id="mobile" readonly name="mobile" type="tel" class="form-control @error('mobile') is-invalid @enderror" value="{{ old('mobile',$purchaseCustomer->mobile) }}" onchange="changeMobile()" autocomplete="mobile" autofocus placeholder="05xxxxxxxx">
                                @error('mobile')
                                <small class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </small>
                                @enderror
                                <p class="text-danger" id="error" role="alert"> </p>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
            ...
            @include('SalesManager.fundingReq.fundingJoint')
        </div>
    ...
    $(document).ready(function() {
        ...
        if ((status != 6 && status != 13) || (statusPay == 0 || statusPay == null || statusPay == 2 || statusPay == 8 || statusPay == 9)) {
            ...
            if (status == 2) { //archived in sales agent
                document.getElementById('archiveWarning1').innerHTML = "<button type='button' class='close' data-dismiss='alert'>&times;</button> {{ MyHelpers::admin_trans(auth()->user()->id,'The request is archived by Sales Agent, you cannot edit anything') }}";
                document.getElementById('archiveWarning1').style.display = "block";
            }
...
*/
    ...
    $('#file-form').submit(function(event) {
        ...
        $.ajax({
            ...
            url: "{{ route('sales.manager.uploadFile')}}",
            data: formData,
            type: 'post',
            async: false,
            processData: false,
            ...
            success: function(response) {
                ...
                $.each(response, function(i, value) { //for loop , I put data as value
                    ...
                    var url = '{{ route("sales.manager.openFile", ":docID") }}';
                    url = url.replace(':docID', docID);
                    var url2 = '{{ route("sales.manager.downFile", ":docID") }}';
    ...
    $(document).on('click', '#delete', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('sales.manager.deleFile') }}", {
                    id: id
    ...
    $(document).on('click', '#send', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('sales.manager.sendFunding')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    var url = '{{ route("sales.manager.fundingRequest", ":reqID") }}';
    ...
    $(document).on('click', '#reject', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('sales.manager.rejectRequest')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
    ...
    $('#frm-update').on('click', '#updatePay', function(e) {
        ...
        $.post("{{ route('sales.manager.updatePrepayment')}}", {
            reqID: reqID,
            real: real,
            incr: incr,
            preval: preval,
    ...
    $(document).on('click', '#approveTsaheel', function(e) {
        ...
        $.get("{{ route('sales.manager.aprroveTsaheel') }}", {
            id: id
        }, function(data) {
            ...
            if (data.status != 0) {
                ...
                document.getElementById('undoTsaheelApprove').innerHTML = "<i class='fa fa-history i-20'></i>";
    ...
    $(document).on('click', '#approveAqar', function(e) {
        ...
        $.get("{{ route('sales.manager.aprroveAqar') }}", {
            id: id
        }, function(data) {
    ...
    $(document).on('click', '#undoApprove', function(e) {
        ...
        $.get("{{ route('sales.manager.undoAqarApprove') }}", {
            id: id
        }, function(data) {
    ...
    $(document).on('click', '#undoTsaheelApprove', function(e) {
        ...
        $.get("{{ route('sales.manager.undoaprroveTsaheel') }}", {
            id: id
        }, function(data) {
    ...
    $(document).on('click', '#rejectPay', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                if (comment != '') {
                    $("#mi-modal3").modal('hide');
                    $.get("{{ route('sales.manager.rejectPrepayment')}}", {
                        id: id,
                        comment: comment
                    }, function(data) {
                        console.log(data);

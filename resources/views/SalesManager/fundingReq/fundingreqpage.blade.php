...
@include('SalesManager.fundingReq.progress')
<br>
...
<form action="{{ route('sales.manager.updateFunding')}}" method="post" novalidate="novalidate" id="frm-update">
    @csrf
                ...
                <div class="col-lg-12">
                    <div class="tableUserOption   flex-wrap downOrder">
                        <div class="addBtn  mt-lg-12 text-center orderBtns">
                            ...
                            {{--
                                <button class="mr-3 warning item" id="archive" role="button" type="button" @if ($reqStatus != 3 && $reqStatus != 7 && $reqStatus != 10 ) disabled style="cursor: not-allowed" @endif>
                                <a @if ($reqStatus == 3 || $reqStatus == 7 || $reqStatus == 10 ) href="{{ route('sales.manager.archFunding',$id)}}" @endif class="text-white" style="text-decoration: none">
                            ...
                            @if ($reqStatus!=5)
                            <button class="mr-3 Cloud item" id="archive" role="button" type="button" @if ($reqStatus !=3 && $reqStatus !=7 && $reqStatus !=10 ) disabled style="cursor: not-allowed" @endif>
                                <a @if ($reqStatus==3 || $reqStatus==7 || $reqStatus==10 ) href="{{ route('sales.manager.archFunding',$id)}}" @endif class="text-white" style="text-decoration: none">
                ...
                <div class="col-lg-7">
                    <div class="tableUserOption   flex-wrap downOrder">
                        <div class="addBtn  mt-lg-12 text-center orderBtns">
                            ...
                            <button class="green item" type="submit" id="update" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Update') }}" @if ($reqStatus !=3 && $reqStatus !=5 && $reqStatus !=7 && $reqStatus !=10 ) disabled style="cursor: not-allowed" @endif>
                                <i class="fas fa-floppy"></i>
                                {{ MyHelpers::admin_trans(auth()->user()->id,'Update') }}
                            </button>
                            <button class="warning item" type="button" id="reject" data-id="{{$id}}" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Reject The Request') }}" @if ($reqStatus !=3 && $reqStatus !=5 && $reqStatus !=7 && $reqStatus !=10 ) disabled style="cursor: not-allowed" @endif>
                                <i class="fas fa-ban"></i>
                                {{ MyHelpers::admin_trans(auth()->user()->id,'Reject The Request') }}
                            </button>
                            <button class="mov item" type="button" id="send" data-id="{{$id}}" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Send The Request') }}" @if ($reqStatus !=3 && $reqStatus !=5 && $reqStatus !=7 && $reqStatus !=10 ) disabled style="cursor: not-allowed" @endif>
    ...
    @if(!empty ($payment) && $purchaseCustomer-> type == 'شراء-دفعة' && ($reqStatus == 5 || $reqStatus == 6 || $reqStatus == 8 || $reqStatus == 13))
    ...
    <div class="tableBar">
        <div class="topRow">
            <div class="row align-items-center text-center text-md-left">
                ...
                @if ($payment->payStatus == 1 || $payment->payStatus == 6 )
                <div class="col-lg-10">
                    <div class="tableUserOption   flex-wrap justify-content-md-end  justify-content-center  ">
                        <div class="addBtn  mt-lg-0 mt-3 orderBtns spansBtn">
                            @if ($payment->isSentSalesAgent == 1)
                            <button class="mr-3 Green" id="rejectPay" data-id="{{$id}}">
                                <span class="toggle" data-toggle="tooltip" data-placement="top" title=" {{ MyHelpers::admin_trans(auth()->user()->id,'Reject Payment') }}">
                                    <i class="fas fa-ban"></i>
                            ...
                            <button class="mr-3 Cloud" id="updatePay" data-id="{{$id}}">
                                <span class="toggle" data-toggle="tooltip" data-placement="top" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Update Payment') }}">
                                    <i class="fas fa-edit"></i>
                                    <span>{{ MyHelpers::admin_trans(auth()->user()->id,'Update Payment') }}</span>
                                </span>
                            </button>
                            <button class="Red2" id="sendPay" type="button" data-id="{{$id}}">
                                <span class="toggle" data-toggle="tooltip" data-placement="top" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Send Payment') }}">
                                    <i class="fas fa-paper-plane"></i>
                                    <span>{{ MyHelpers::admin_trans(auth()->user()->id,'Send Payment') }}</span>
                                </span>
                            </button>
                        </div>
                    </div>
                ...
                <div class="col-lg-10">
                    <div class="tableUserOption   flex-wrap justify-content-md-end  justify-content-center  ">
                        <div class="addBtn  mt-lg-0 mt-3 orderBtns spansBtn">
                            @if ($payment->isSentSalesAgent == 1)
                            <button class="mr-3 Green" id="rejectPay" data-id="{{$id}}" disabled style="cursor: not-allowed">
                                <span class="toggle" data-toggle="tooltip" data-placement="top" title=" {{ MyHelpers::admin_trans(auth()->user()->id,'Reject Payment') }}">
                                    <i class="fas fa-ban"></i>
                                    <span> {{ MyHelpers::admin_trans(auth()->user()->id,'Reject Payment') }}</span>
                                </span>
                            ...
                            <button class="mr-3 Cloud" id="updatePay" data-id="{{$id}}" disabled style="cursor: not-allowed">
                                <span class="toggle" data-toggle="tooltip" data-placement="top" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Update Payment') }}">
                                    <i class="fas fa-edit"></i>
                                    <span>{{ MyHelpers::admin_trans(auth()->user()->id,'Update Payment') }}</span>
                                </span>
                            </button>
                            <button class="Red2" id="sendPay" data-id="{{$id}}" type="button" disabled style="cursor: not-allowed">
                                <span class="toggle" data-toggle="tooltip" data-placement="top" title="{{ MyHelpers::admin_trans(auth()->user()->id,'Send Payment') }}">
                                    <i class="fas fa-paper-plane"></i>
                                    <span>{{ MyHelpers::admin_trans(auth()->user()->id,'Send Payment') }}</span>
                                </span>
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    @else
                    <ul style="display: none" class="list-unstyled d-flex flex-wrab align-items-center justify-content-center topRow">
                    ...
                    <div class="tabs-serv">
                        <div class="tab-body">
                            ...
                            <div class="row hdie-show display-none" id="content5-cont">
                                <div class="col-lg-12   mb-md-0">
                                    @include('SalesManager.fundingReq.tsaheel')
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="row hdie-show display-none" id="content5-cont">
                                <div class="col-lg-12   mb-md-0">
                                    @include('SalesManager.fundingReq.updatePage')
                                </div>
                            </div>
                            @endif
                        </div>
            ...
            <div>
                <label style="width: 100%; display: block;" for="tab1">
                    <span>
                        @include('SalesManager.fundingReq.fundingReqInfo')
                    </span>
                </label>
            </div>
                    ...
                    <span>
                        <div class="userFormsContainer mb-3">
                            <div class="userFormsDetails topRow">
                                @if ($purchaseCustomer-> is_canceled == 0 && ($reqStatus == 3 ||$reqStatus == 5 || $reqStatus == 7 || $reqStatus == 10 ))
                                <div class="row">
                                    @if ($followdate != null)
...
@section('updateModel')
@include('SalesManager.fundingReq.req_records')
@include('SalesManager.fundingReq.documentModel')
@endsection

@section('confirmMSG')
@include('SalesManager.fundingReq.confirmationMsg')
@include('SalesManager.fundingReq.confirmSendingMsg')
@include('SalesManager.fundingReq.confirmRejectMsg')
@include('SalesManager.fundingReq.confirmSendingMsgPay')
@include('SalesManager.fundingReq.SendingModel')
@endsection

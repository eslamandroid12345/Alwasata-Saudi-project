  ...
  <div class="card">
    ...
    <div class="card-body card-block">
      ...
      <form action="{{ route('all.task_post')}}" method="post" class="">
        ...
        <select id="recived" style="width: 100%" onfocus='this.size=3;' onblur='this.size=1;' onchange='this.size=1;' class="form-control @error('sex') is-invalid @enderror" name="recived">


          <option value="">---</option>
            @if(auth()->user()->role ==13)
                @foreach($allUsers as $user)
                    <option value="{{$user->user->id}}" @if (old('recived') == $user->user->id) selected="selected" @endif>{{$user->user->role == 0 ? "إستشاري" : "مشرف التمويل"}} | {{ $user->user->name}}</option>
                @endforeach
            @else
                @foreach($allUsers as $user)
                    <option value="{{$user->id}}" @if (old('recived') == $user->id) selected="selected" @endif>{{($user->role == 5)? $user->name.' : '.$user->name_for_admin : $user->name}} - {{/*$user*/''}}</option>
                @endforeach
            @endif

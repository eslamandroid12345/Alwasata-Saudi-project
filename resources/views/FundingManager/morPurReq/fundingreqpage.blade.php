...
<form action="{{ route('funding.manager.updateFunding')}}" method="post" novalidate="novalidate">
    @csrf
        ...
        <div class="topRow no-radius">
            <div class="row align-items-center text-center text-md-left">
                ...
                <div class="col-lg-7">
                    <div class="tableUserOption   flex-wrap justify-content-md-end  justify-content-center downOrder">
                        <div class="addBtn  mt-lg-0 mt-3 orderBtns">
                            <button class="mr-3 warning item" role="button" type="button" id="reject" data-id="{{$id}}" @if( $reqStatus != 21 && $reqStatus != 25) disabled style="cursor: not-allowed" @endif >
                                <i class="fas fa-trash mr-2"></i>
                                {{ MyHelpers::admin_trans(auth()->user()->id,'Reject The Request') }}
                            </button>

                            <button class="green item" type="submit" id="update"  @if( $reqStatus != 21 && $reqStatus != 25) disabled style="cursor: not-allowed" @endif  title="{{ MyHelpers::admin_trans(auth()->user()->id,'Update') }}">
                                <i class="fas fa-edit"></i>
                                {{ MyHelpers::admin_trans(auth()->user()->id,'Update') }}
                            </button>

                            <button class="mov item" type="button" id="approve"  data-id="{{$id}}" @if( $reqStatus != 21 && $reqStatus != 25) disabled style="cursor: not-allowed" @endif    title="{{ MyHelpers::admin_trans(auth()->user()->id,'Send The Request') }}">
        ...
        <div class="container-fluid">
            ...
            <div>
                <label style="width: 100%; display: block;" for="tab1">
                    <span>
                         @include('FundingManager.morPurReq.fundingReqInfo')
                    </span>
                </label>
...
@section('updateModel')
    @include('FundingManager.morPurReq.req_records')
    @include('FundingManager.morPurReq.documentModel')
@endsection

@section('confirmMSG')
    @include('FundingManager.morPurReq.confirmationMsg')
    @include('FundingManager.morPurReq.confirmSendingMsg')
    @include('FundingManager.morPurReq.confirmRejectMsg')
    @include('FundingManager.morPurReq.SendingModel')
@endsection

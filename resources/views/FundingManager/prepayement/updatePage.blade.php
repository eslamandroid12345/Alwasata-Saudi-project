...
@if ($payment->payStatus == 0 || $payment->payStatus == 3)
<!-- Status of payment-->

<form action="{{ route('funding.manager.updatePrepayment')}}" method="post" novalidate="novalidate">
    @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-2">
                        <a href="{{ route('funding.manager.cancelPrepayment',$id)}}">
                            <button type="button" id="cancel" class="btn btn-block btn-outline-danger"><i class="fa fa-trash"></i> Cancel Payment</button>
                        </a>
                    </div>
                    ...
                    <div class="col-4">
                        <button type="button" id="send" data-id="{{$id}}" class="btn btn-info btn-block"><i class="fa fa-send"></i>Send</button>
                    </div>
        ...
        <label class="fnnn" style="width: 100%; display: block;background: #fff;color: #999;border-bottom: 2px solid #f0f0f0;" for="tab1">
            <span>
                ...
                <div class="row">
                    ...
                    <div class="col-4">
                        <a href="{{ route('funding.manager.fundingRequest',$id)}}">
                    ...
                    <div class="col-4">
                        <a href="{{ route('funding.manager.morPurRequest',$id)}}">
                            <button type="button" class="btn btn-block btn-outline-primary"><i class="fa fa-credit-card"></i>View Request</button>
                        </a>
                    </div>
        ...
        <label class="fnnn" style="width: 100%; display: block;background: #fff;color: #999;border-bottom: 2px solid #f0f0f0;" for="tab1">
            <span>
                ...
                <div class="row">
                    ...
                    <div class="col-2">
                        <a href="{{ route('funding.manager.cancelPrepayment',$id)}}">
                            <button disabled type="button" id="cancel" class="btn btn-block btn-outline-danger"><i class="fa fa-trash"></i> Cancel Payment</button>
                        </a>
                    </div>
                    <div class="col-2">
                    </div>
...
@section('confirmMSG')
@include('FundingManager.prepayement.confirmSendingMsg')
@endsection
...
<script>
    ...
    $(document).ready(function() {
        ...
        if (status == 1) { //sending to general manager
            document.getElementById('sendingWarning').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button> The prepayment has sent to sales Manager";
            document.getElementById('sendingWarning').style.display = "block";
        }

        if (status == 2) { //canceled payment
            document.getElementById('archiveWarning').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button> The prepayment canceled.. <a href='{{ route('funding.manager.restPrepayment',$id)}}'> Restore?</a>";
            document.getElementById('archiveWarning').style.display = "block";
        }

        if (status == 3) { //canceled payment
        ...
        if (status == 4) { //send to agent 
            document.getElementById('sendingWarning').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button> The prepayment send to agent";
            document.getElementById('sendingWarning').style.display = "block";
        }
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.get("{{ route('funding.manager.sendPrepayment')}}", {
                    ...
                    var url = '{{ route("funding.manager.updatePrepaymentPage", ":reqID") }}';

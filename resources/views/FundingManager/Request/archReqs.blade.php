...
<div class="middle-screen">
    <h2 style=" text-align: center;font-size: 20pt;">لايوجد طلبات في المتابعة</h2>
</div>
...
@include('FundingManager.Request.confirmRestMsg')
@endsection
    ...
    function archiveAllReqs(array) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('funding.manager.restReqArray')}}", {
                    array: array,
                    _token: "{{csrf_token()}}",
                }, function(data) {
                    console.log(data);
                    var url = '{{ route("funding.manager.archRequests") }}';
    ...
    */
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ url('fundingManager/archreqs-datatable') }}",
                'data': function(data) {
            ...
            createdRow: function(row, data, index) {
                ...
                $('td', row).eq(7).attr('title', data.fm_comment); // to show other text of comment

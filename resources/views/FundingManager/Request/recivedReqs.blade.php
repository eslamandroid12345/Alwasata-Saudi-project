...
@include('FundingManager.Request.confirmArchMsg')
@endsection
...
function archiveAllReqs(array) {
    ...
    modalConfirm(function(confirm) {
        if (confirm) {
            ...
            $.post("{{ route('funding.manager.archReqArray')}}", {
                array: array,
                _token: "{{csrf_token()}}",
            }, function(data) {

                var url = '{{ route("funding.manager.recivedRequests") }}';
    ...
    */
    ...
    $(document).ready(function() {
        var dt = $('.data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                'url': "{{ url('fundingManager/recivedreqs-datatable') }}",
                'data': function(data) {
            ...
            createdRow: function(row, data, index) {
                ...
                $('td', row).eq(8).attr('title', data.fm_comment); // to show other text of comment

<div class="userFormsInfo  ">
    ...
    <div class="userFormsContainer mb-3">
                ...
                <div  id="tableAdminOption" class="row">
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <input readonly  id="mobile" name="mobile" type="tel" class="form-control @error('mobile') is-invalid @enderror" value="{{ old('mobile',$purchaseCustomer->mobile) }}" onchange="changeMobile()" autocomplete="mobile" autofocus placeholder="05xxxxxxxx">
                            @error('mobile')
                            <small class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </small>
        ...
        @include('FundingManager.fundingReq.fundingJoint')
    </div>
...
<script>
 ...
 function checkObligation(that) {
    ...
    $(document).on('click', '#send', function(e) {
        ...
        if (statusPay == 6) { // the prepayment is rejected
            var modalConfirm = function(callback) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('funding.manager.sendFunding')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    var url = '{{ route("funding.manager.fundingRequest", ":reqID") }}';
    ...
    $(document).on('click', '#reject', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.post("{{ route('funding.manager.rejectRequest')}}", {
                    id: id,
                    comment: comment
                }, function(data) {
                    var url = '{{ route("funding.manager.fundingRequest", ":reqID") }}';
    ...
    $(document).ready(function() {
        ...
        if (statusReq == 6 || statusReq == 13) {
            ...
            if ((status == 2 || status == 8) && (statusReq == 6 || statusReq == 13)) { //canceled payment
                document.getElementById('archiveWarning').innerHTML = " <button type='button' class='close' data-dismiss='alert'>&times;</button> {{ MyHelpers::admin_trans(auth()->user()->id,'The prepayment canceled') }}   , <a href='{{ route('funding.manager.restPrepayment',$id)}}'> {{ MyHelpers::admin_trans(auth()->user()->id,'Recancele') }}؟ </a>";
                document.getElementById('archiveWarning').style.display = "block";
            }
    ...
    $(document).on('click', '#appPay', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.get("{{ route('funding.manager.appPrepayment')}}", {
                    id: id
                }, function(data) {
                    var url = '{{ route("funding.manager.fundingRequest", ":reqID") }}';
                    url = url.replace(':reqID', data.id);
    ...
    $('#frm-update').on('click', '#updatePay', function(e) {
        ...
        $.post("{{ route('funding.manager.updatePrepayment')}}", {
            reqID: reqID,
            real: real,
            incr: incr,
            preval: preval,
    ...
    $(document).on('click', '#rejectPay', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                if (comment != '') {
                    $("#mi-modal3").modal('hide');
                    $.get("{{ route('funding.manager.rejectPrepayment')}}", {
                        id: id,
                        comment: comment
                    }, function(data) {
                        console.log(data);
    ...
    $(document).on('click', '#sendPay', function(e) {
        ...
        modalConfirm(function(confirm) {
            if (confirm) {
                ...
                $.get("{{ route('funding.manager.sendPrepayment')}}", {
                    id: id
                }, function(data) {
                    var url = '{{ route("funding.manager.fundingRequest", ":reqID") }}';
                    url = url.replace(':reqID', data.id);
...
<script type="text/javascript">
    ...
    /////////////////////////////////////////
    $(document).on('click', '#send-bank', function(e) {
        $("#mi-modal6").modal('show');
    });
    $('body').on('click', '.ajax-req-bank', function(e) {
        var u = $(this).data('route');
        var o = $(this);
        var P_element = o.parent().parent().parent();
        var P_element_close = o.closest('.modal-body');
        if(o.data('val') == 'close' )
        {
            swal({
                    title: "تاكيد افراغ الطلب!",
                    text: "هل انت متاكد من افراغ الطلب سيتم افراغ الطلب عند جميع موظفين البنك؟",
                    icon: "warning",
                    buttons: ["اغلاق",'افراغ'],
                    dangerMode: true,
                })
                ...
                if (OkClose){
                    P_element.html(`
                        جاري التنفيذ<i class="fa fa-spinner"></i>
                    `);
                    $.get(u)
                    .done(function(res){
                        P_element_close.html(res);
                    })
                    .fail(function(er){
                        console.log(er);
                    });

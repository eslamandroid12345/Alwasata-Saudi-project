    ...
    <div class="main-content-page-grid">
        <div class="main-content-page">
            ...
            <div class="col-12">
              <div class="d-flex align-items-center justify-content-between">
                ...
                {{-- <ol class="breadcrumb mb-0">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">العنوان</li>
                </ol> --}}
                <!-- end::breadcrumb  -->
              </div>
            </div>
          ...
          <div class="row pb-3">
            <!-- begin::col  -->
            <div class="col-12">
              <div class="d-flex align-items-center justify-content-center bg-white border rounded p-2">
                {{-- <h6 class="ms-3 font-medium">بدون اسم - 598765432</h6> --}}
                <h4 class="text-center" style="color:#1d406e"><span style="font-size: 1.17em;font-weight: bolder">{{$reqInfo->name}}</span> - {{$reqInfo->mobile}}</h4>
                @if ($sentrole ==5)
                ...
                @else
                    ...
                    @endif
                @endif
                <button type="button" class=" btn fnfn mx-2" style="background-color: #00ACF1;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="15.513" height="11.555" viewBox="0 0 15.513 11.555">
                        <g id="Icon_feather-eye" data-name="Icon feather-eye" transform="translate(-1 -5.5)">
                          <path
                            id="Path_4026"
                            data-name="Path 4026"
                            d="M1.5,11.278S4.139,6,8.757,6s7.257,5.278,7.257,5.278-2.639,5.278-7.257,5.278S1.5,11.278,1.5,11.278Z"
                            fill="none"
                            stroke="#fff"
                          ...
                          <path
                            id="Path_4027"
                            data-name="Path 4027"
                            d="M17.458,15.479A1.979,1.979,0,1,1,15.479,13.5,1.979,1.979,0,0,1,17.458,15.479Z"
                            transform="translate(-6.722 -4.201)"
                            fill="none"
                            stroke="#fff"
                            stroke-linecap="round"
                            stroke-linejoin="round"
            ...
            <!-- begin::col  -->
            <div class="col-12">
              <div class="timeline-wrapper my-4 py-lg-3">
                <div class="timeline-wrapper-inner">
                    ...
                    @foreach($task_contents as $task_content)
                        <div class="timeline-item">
                            <div class="timeline-date">
                                <span class="text-block">{{Carbon\Carbon::parse($task_content->date_of_content )->format('Y-m-d')}}</span>
                                <span class="text-block">{{Carbon\Carbon::parse($task_content->date_of_content )->format('H:i:s')}} </span>
                            </div>
                            <div class="timeline-line"></div>
                            <div class="timeline-content">
                                @if ($tasks->sentrole == 5 && (auth()->user()->role == 7 || auth()->user()->role == 4))
                                    <h4 class="timeline-title font-medium mb-2">{{$tasks->sentname}}</h4>
                                @elseif ($tasks->sentrole == 5 && (auth()->user()->role != 7 && auth()->user()->role != 4))
                                    <h4 class="timeline-title font-medium mb-2">{{$tasks->sentname}}</h4>
                                @else
                                    <h4 class="timeline-title font-medium mb-2">{{$tasks->sentname}}</h4>
                                @endif
                                <h5 class="timeline-desc"> {{$task_content->content}}</h5>
                            </div>
                        </div>
                        ...
                        @if($task_content->user_note != null)
                            <div class="timeline-item">
                                <div class="timeline-date">
                                    <span class="text-block">{{Carbon\Carbon::parse($task_content->date_of_note )->format('Y-m-d')}}</span>
                                    <span class="text-block">{{Carbon\Carbon::parse($task_content->date_of_note )->format('H:i:s')}} </span>
                                </div>
                                <div class="timeline-line"></div>
                                <div class="timeline-content">
                                    <h4 class="timeline-title font-medium mb-2">{{$tasks->recname}}</h4>
                                    <h5 class="timeline-desc"> {{$task_content->user_note}}</h5>
                                </div>
                            </div>
                        @endif
                    @endforeach
                  ...
                  <div class="timeline-footer">
                        @if($tasks->status == 3)
                            <div class="timeline-completet font-medium">{{ MyHelpers::admin_trans(auth()->user()->id,'The task is completed') }} </div>
                        @elseif( $tasks->status == 4)
                            <div class="timeline-completet font-medium" style="color:red">{{ MyHelpers::admin_trans(auth()->user()->id,'The task is not completed') }}</div>
                        @elseif( $tasks->status == 5)
                            <div class="timeline-completet font-medium" style="color:yellowgreen">{{ MyHelpers::admin_trans(auth()->user()->id,'The task is canceled') }}</div>
                        @endif
                  </div>

...
<div class="afnan">
  <div class="container text-center" style="text-align:center;">
    ...
    @if($currentTasks->sentid == auth()->user()->id)
    <h3>{{$currentTasks->recname}}</h3>

    @if($currentTasks->recrole == 0)
    <p>استشاري المبيعات</p>
    @elseif($currentTasks->recrole == 1)
    <p>مدير المبيعات</p>
    @elseif($currentTasks->recrole == 2)
    <p>مدير التمويل</p>
    @elseif($currentTasks->recrole == 3)
    <p>مدير الرهن</p>
    @elseif($currentTasks->recrole == 4)
    <p>المدير العام </p>
    @elseif($currentTasks->recrole == 5)
    <p> @if ($currentTasks->recname =='الجودة') قسم الجودة @else قسم المتابعة @endif</p>
    @elseif($currentTasks->recrole == 6)
    <p> متعاون</p>
    @elseif($currentTasks->recrole == 7)
    <p> مدير النظام</p>
    @endif
    ...
    @if ($currentTasks->sentrole == 5 && (auth()->user()->role == 7 || auth()->user()->role == 4))
    <h2>{{$currentTasks->sentname}}</h2>
    @elseif ($currentTasks->sentrole == 5 && (auth()->user()->role != 7 && auth()->user()->role != 4))
    <h2></h2>
    @else
    <h2>{{$currentTasks->sentname}}</h2>
    @endif
    ...
    @if($currentTasks->sentrole == 0)
    <p>استشاري المبيعات</p>
    @elseif($currentTasks->sentrole == 1)
    <p>مدير المبيعات</p>
    @elseif($currentTasks->sentrole == 2)
    <p>مدير التمويل</p>
    @elseif($currentTasks->sentrole == 3)
    <p>مدير الرهن</p>
    @elseif($currentTasks->sentrole == 4)
    <p>المدير العام </p>
    @elseif($currentTasks->sentrole == 5)
    <p> @if ($currentTasks->sentname =='الجودة') قسم الجودة @else قسم المتابعة @endif</p>
    @elseif($currentTasks->sentrole == 6)
    <p> متعاون</p>
    @elseif($currentTasks->sentrole == 7)
    <p> مدير النظام</p>
    @endif
      ...
      <a href="{{route('funding.manager.fundingRequest',$reqInfo->req_id)}}">
        ...
        <a href="{{route('mortgage.manager.fundingRequest',$reqInfo->req_id)}}">
          ...
          <a href="{{route('general.manager.fundingRequest',$reqInfo->req_id)}}">
            ...
            <a href="{{route('quality.manager.fundingRequest',$currentTasks->req_id)}}">
                      ...
                      <a href="{{route('funding.manager.fundingRequest',$reqInfo->id)}}">
                        ...
                        <a href="{{route('mortgage.manager.fundingRequest',$reqInfo->id)}}">
                          ...
                          <a href="{{route('general.manager.fundingRequest',$reqInfo->id)}}">
                            @elseif(auth()->user()->role == 5)
                            <a href="{{route('quality.manager.fundingRequest',$currentTasks->id)}}">
                              @elseif(auth()->user()->role == 7)
...
<hr>
@foreach($getReqHistories as $getReqHistorie)

@if ($task->recid == $getReqHistorie->recive_id)
<p style="text-align: center;"> تم نقل الطلب إلى {{$task->recname}} في تاريخ {{$getReqHistorie->history_date}}</p>
@endif
...
<section id="cd-timeline" class="cd-container">
  ...
  <?php $task_contents = App\Http\Controllers\TaskController::getContentOfTask($task->id); ?>
  ...
  <div class="cd-timeline-block">
    ...
    <div class="cd-timeline-content" style=" background: rgb(86, 115, 148,0.6);">
      @if ($task->sentrole == 5 && (auth()->user()->role == 7 || auth()->user()->role == 4))
      <h2>{{$task->sentname}}</h2>
      @elseif ($task->sentrole == 5 && (auth()->user()->role != 7 && auth()->user()->role != 4))
      <h2>{{$task->sentname}}</h2>
      @else
      <h2>{{$task->sentname}}</h2>
      @endif
...
<form action="{{ route('all.update_users_task_content')}}" method="post" class="">
  ...
  <!--For user_note-->
  @if ($currentTasks->recid == auth()->user()->id)
  <form action="{{ route('all.update_users_task_note')}}" method="post" class="">
    ...
    <div class="row">
      ...
      @if($currentTasks->status==0 || $currentTasks->status==1 ||$currentTasks->status==2)
        ...
        <div class="card text-center">
          ...
          <div class="card-footer">
            <div class="row">
              ...
              @if($currentTasks->status==0 || $currentTasks->status==1)

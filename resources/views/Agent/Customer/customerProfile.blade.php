...
<div class="row">
    ...
    <div class="card card-stats">
      ...
      <div class="card-footer">
        <div class="stats" style="font-size:small;">
          <i class="material-icons text-primary">add_circle_outline</i><a href="{{ route('agent.purchasePage', ['title' => 'funding', 'id' => $customer->id])}}">{{ MyHelpers::admin_trans(auth()->user()->id,'Add') }}</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="col-lg-3 col-md-6 col-sm-6">
    <div class="card card-stats">
      ...
      <div class="card-footer">
        <div class="stats" style="font-size:small;">
          <i class="material-icons text-primary">add_circle_outline</i><a href="{{ route('agent.purchasePage',['title' => 'mortgage', 'id' => $customer->id])}}">{{ MyHelpers::admin_trans(auth()->user()->id,'Add') }}</a>
        </div>
      </div>
    </div>
  </div>
...
$(document).ready( function () {
    ...
    $('#purRequests_table').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax:  "{{ url('agent/customerprofile-purRequests-datatable/') }}" +'/'+ customer_id ,
        columns: [
            { data: 'id', name: 'id' },
    ...
    $('#morRequests_table').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: "{{ url('agent/customerprofile-morRequests-datatable/') }}" +'/'+ customer_id ,
        columns: [
            { data: 'id', name: 'id' },
            { data: 'req_date', name: 'req_date' },
            { data: 'type', name: 'type' },
    ...
    $('#morPurRequests_table').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax:  "{{ url('agent/customerprofile-morPurRequests-datatable/') }}" +'/'+ customer_id ,
        columns: [
            { data: 'id', name: 'id' },
            { data: 'req_date', name: 'req_date' },
            { data: 'type', name: 'type' },
    ...
    $('#payRequests_table').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: "{{ url('agent/customerprofile-payRequests-datatable/') }}" +'/'+ customer_id ,
        columns: [
            { data: 'id', name: 'id' },

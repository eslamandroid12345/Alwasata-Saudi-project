{{-- For Search style   --}}
<div class="topRow" >
    <!--<form name="filter" id="filter" method="get" action="{{ route('requestSourcesChartR') }}">-->
    <form name="filter" id="filter" method="get">
        <div class="row align-items-center text-center text-md-left">
            <div class="col-4">

                <label class="label">{{ MyHelpers::admin_trans(auth()->user()->id,'From Date') }}</label>
                <input class="form-control" type="date" name="startdate" value="{{ (app('request')->input('startdate')) != null ? (app('request')->input('startdate')) : date('Y-m-d', strtotime('-30 day'))  }}">

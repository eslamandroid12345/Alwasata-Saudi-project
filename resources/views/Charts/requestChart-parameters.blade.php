...
{{-- For Search style   --}}
<div class="topRow" >
    <form name="filter" id="filter" method="get"  @if ($manager_role == 7 || $manager_role == 4) action="{{ route('requestChartR') }}" @elseif ($manager_role == 1) action="{{ route('sales.manager.requestChartRForSalesManager') }}" @elseif ($manager_role == 11) action="{{ route('training.requestChartRForTraining') }}" @else action="{{ route('agent.requestChartRForAgent') }}" @endif  >
        <div class="row align-items-center text-center text-md-left">
            ...
            @if ($manager_role != 1 && $manager_role != 0 && $manager_role != 11)
                <div class="col-12">
                        ...
                        <div class="rs-select2 js-select-simple select--no-search">
                            <select class="form-control" name="manager_id[]" multiple required id="manager_id">
            ...
            @if ($manager_role == 1 || $manager_role == 11)
                <div class="col-12">
            ...
            <div class="col-12">
                @include('Charts.requestChart-status')
            </div>

            <div class="col-12">
                @include('Charts.requestChart-class')
            </div>

            <div class="col-12">
                @include('Charts.requestChart-basket')
            </div>
        </div>

...
@section('title')
{{ MyHelpers::admin_trans(auth()->user()->id,'Charts') }} - {{ MyHelpers::admin_trans(auth()->user()->id,'final result') }}
@endsection
...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        @if ($manager_role != 0)
            <h3>{{ MyHelpers::admin_trans(auth()->user()->id,'final result') }}:</h3>
        @else
            <h3>{{ MyHelpers::admin_trans(auth()->user()->id,'agent_assments') }}:</h3>
        @endif
    </div>
</div>

{{-- For Search Parameters   --}}
@include('Charts.finalResultChart-parameters')

<div class="main-content">
    <div class="section_content section_content--p30">
        <div class="container-fluid">
            @if($data_for_chart != null)
            @include('Charts.finalResultChart-table')
            @include('Charts.finalResultChart-chart')
            @endif
        </div>
    </div>
</div>

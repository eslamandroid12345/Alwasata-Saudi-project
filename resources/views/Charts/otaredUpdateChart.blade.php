...
@section('title')
{{ MyHelpers::admin_trans(auth()->user()->id,'Charts') }} -  {{ MyHelpers::admin_trans(auth()->user()->id,'Otared Update') }}
@endsection
...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        <h3> {{ MyHelpers::admin_trans(auth()->user()->id,'Otared Update') }}:</h3>
    </div>
</div>

{{-- For Search Parameters   --}}
@include('Charts.otaredUpdateChart-parameters')

<div class="main-content">
    <div class="section_content section_content--p30">
        <div class="container-fluid">
            @if($data_for_chart != null)
            @include('Charts.otaredUpdateChart-table')
            @endif
        </div>

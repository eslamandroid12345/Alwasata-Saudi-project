...
<div class="addUser my-4">
    <div class="userBlock d-flex align-items-center justify-content-between flex-wrap">
        <h3> مصادر المعاملات - الوساطة:</h3>
    </div>
    <div class="alert alert-info mt-3">
        <b>تنبيه </b>
        تقرير مصادر المعاملات الوساطة المعلقة بالتاريخ (يعمل التقرير بشكل سريع لمدة 30 يوم ماضية)
    </div>
</div>
...
@include('Charts.sources.form-wsata')
<div class="tableBar">
        ...
        <div class="row">
            @foreach($sources as $source)
                <span class="button-checkbox">
                    <button type="button" class="btn btn-sm" data-color="primary">{{$source->value}}</button>
                    <input type="checkbox" value="{{$loop->iteration+1}}" class="hidden fields" checked name="fields"/>
                </span>
            @endforeach
            @foreach($collaborators as $source)
            <span class="button-checkbox">
                <button type="button" class="btn btn-sm" data-color="primary">{{$source->name}}</button>
                <input type="checkbox" value="{{count($sources)+$loop->iteration+1}}" class="hidden fields" checked name="fields"/>
            </span>
            @endforeach
        </div>
    </div>
   {{-- <button onclick="myFun(5)">ClickMe</button>--}}
    <div class="dashTable row" style="overflow: hidden">
        ...
        <div class="col-lg-8 pl-0 pr-5">
            <input type="text" class="form-control ml-3" max="30" id="count" placeholder="عدد الأيام">
    ...
    <div class="dashTable">
        <table id="pendingReqs-table" class="table table-bordred table-striped data-table">
            ...
            <tr id="tfoot">
                ...
                <th>المجموع</th>
                @foreach($sources as $source)
                    <th id="source{{$source->id}}"><i class="fa fa-spinner fa-spin"></i></th>
                @endforeach
                @foreach($collaborators as $user)
                    <th id="user{{$user->id}}"><i class="fa fa-spinner fa-spin"></i></th>
                @endforeach
                <th>#</th>
            </tr>
            </tfoot>
...
<script>
    ...
    var dt = $('.data-table').DataTable({
        ...
        serverSide: true,
        ajax: ({
           'url': "{{ route('report.sources.wsata')  }}",
           'method': 'GET',
           'data':function (data) {
        ...
        columns: [
            ...
            @foreach($sources as $source)
            {
                ...
                "render": function (data, type, row, meta) {
                    ...
                    data.forEach(function (arrayItem) {
                        ...
                        array.push(arrayItem.source)
                        counts.push(arrayItem.counts)
                    });
                    var key=array.indexOf("{{$source->id}}");
            ...
            {
                "data": "collaborates",
                "name": "collaborates{{$collaborator->id}}",
                "render": function (data, type, row, meta) {
                    var id=0;
                    var array = [];
                    var counts = [];
                    var data= JSON.parse(data)
                    data.forEach(function (arrayItem) {
                        array.push(arrayItem.collaborator_id)
                        counts.push(arrayItem.counts)
                    });
                    var key=array.indexOf({{$collaborator->id}});
        ...
        initComplete: function() {
            ...
            $("#many").on('click', function (e) {
                ...
                if($("#count").val() > 30){
                    swal({
                        title: 'خطأ!',
                        text: 'خطأ , القصي قيمة مسموح بها 30 يوم',
                        type: 'error',
                        timer: '1500'
                    })
                }else{
    ...
    function ajaxCall(key) {
        if(key.indexOf("source") != -1){
            $.ajax({
                url: "{{ route('report.sources.wsata.sum')  }}",
                type: "GET",
                data: {
                    'type' : "sources",
                    'model' : "Request",
...
<script>
    ...
    $('#enddate').change(function () {
        ...
        var today1 = now.getFullYear()+"-"+(month2)+"-"+(day1) ;
        //$("#startdate").attr("max",today1).attr("min",$(this).val())
    })
    ...
    $('#startdate').change(function () {
        ...
        var today1 = now.getFullYear()+"-"+(month2)+"-"+(day1) ;
        //$("#enddate").attr("max",today1).attr("min",$(this).val())
    })
    $('#status_user').change(function () {
        reFullAdviser_id()
    })
    ...
    $(function () {
        $('.button-checkbox').each(function () {
            ...
            $button.on('click', function () {
                ...
                dt.column( {{count($sources) +count($collaborators) +2}} ).visible(true)

<script>
    ...
    function submitDelete() {
        ...
        $.ajax({
            url: "{{ url('HumanResource/phones/') }}" + '/' + id,
            type: "POST",
            data: {
                '_method': 'DELETE',
                '_token': "{{csrf_token()}}"
    ...
    function addForm() {
        ...
        $.ajax({
            url: "{{ url('HumanResource/phones') }}" + '/' + {{$user->id}} + "/edit",
            type: "GET",
            data:{ 'user_id':'user'},
            dataType: "JSON",
            success: function(data) {
                var edit = $('#edit-data');
                edit.html("");
                document.querySelector('#checkMobile2').innerHTML = "<i class='fa fa-question i-20'></i>  {{ MyHelpers::admin_trans(auth()->user()->id,'Check') }}";
    ...
    function editForm(id) {
        ...
        $.ajax({
            url: "{{ url('HumanResource/phones') }}" + '/' + id + "/edit",
            type: "GET",
            dataType: "JSON",
            success: function(data) {
    ...
    $('#savePhones').on('click', function(e) {
        ...
        if(error==false){
            if (!e.isDefaultPrevented()) {
                url = "{{ url('HumanResource/phones-updates') }}";
                $.ajax({
                    url: url,
                    type: "POST",
                    data: {
    ...
    $(document).on('click', '#checkMobile2', function(e) {
        ...
        if (mobile != null /*&& regex.test(mobile)*/) {
            ...
            document.getElementById('add_mobile_error').innerHTML = "أدخل رقم جوال يبدأ ب 05";
    ...
    $(document).on('click', '#checkMobile', function(e) {
      ...
      //  var regex = new RegExp(/^(05)(5|0|3|6|4|9|1|8|7)([0-9]{7})$/);*/
        ...
        if (mobile != null/* && regex.test(mobile)*/) {
            document.getElementById('mobile-error-show').innerHTML = "";
            $.post("{{ route('HumanResource.checkMobile') }}", {
                mobile: mobile,
                _token: "{{csrf_token()}}"
            }, function(data) {
                ...
                if (data.errors) {
                    if (data.errors.mobile) {
                        $('#mobile-error-show').html(data.errors.mobile[0])
                    }
                } if (data == "no") {

...
class Agent2Controller extends Controller
{
    public function index()
    {
        $customers = Customer::whereHas('request')->where('user_id',auth()->user()->id)->whereNotNull("app_rate_starts")->count();
        $rated_customers = Customer::whereHas('request')->where('user_id',auth()->user()->id)->whereNotNull("app_rate_starts")->get();
        ...
        return view("Agent.Rates.index",compact("customers","salesAgents"));
    }
    ...
    public function datatable()
    {
        ...
        $customers = Customer::query()->whereHas('request')->where('user_id',auth()->user()->id)->whereNotNull("app_rate_starts")->orderBy('date_of_rate', 'DESC')/*->get()*/;
        ...
        ->addColumn('action', function ($row) {
            $data = '<div class="tableAdminOption"><span type="span" class="item pointer" data-toggle="tooltip" data-placement="top" title="عرض تفاصيل الطلب">
                            <a href="'.route('agent.fundingRequest', $row->request->id).'"> <i class="fa fa-eye"></i></a>
                      </span>
